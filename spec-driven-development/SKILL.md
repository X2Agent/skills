...
- **谨慎使用本技能**: 仅改文案、修一个显而易见的小 bug、做纯机械重命名时，不必强制产出完整规范文档；这类小改动通常可以不启用本技能/退出本流程直接修改；若仍选择使用本技能，则需先给出最小化说明并经用户确认后再修改。
- **进入实现前的标准**: 在启用本技能时，只有当需求边界、接口契约、行为条目和错误处理都已明确，并经用户确认后，才进入实现阶段。
...
2. **代码质量检查**：确认命名、结构、错误处理与项目约定一致；如有需要，可进行一次代码规范/质量自检或请同伴 review
...
- 规范文档不要停留在“需求摘要”，而要写到可直接驱动测试与实现的粒度
- 每个条目都要能回答：做什么、何时触发、预期结果是什么、失败时怎样表现
- 若实现周期较长，必须持续更新规范状态与进度表，避免文档失效
- 对于非常小的修改，可保留 SDD 思想，但采用轻量版：简短边界说明 + 最小测试 + 最小实现
